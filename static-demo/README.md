...
**1. 声音。** 体验版自带真实英文语音（`tools/tts_lesson.mjs` 用本地 TTS 生成），**默认直接播放音频文件**，
不依赖访问者设备的语音合成能力——安卓微信的内置浏览器经常没有英文语音包，靠 TTS 会哑火。

音色是机器音，演示够用。想换成老师本人的录音：替换 `public/assets/lesson1.mp3`，
并把 `mock-api.js` 里各句的 `startMs` / `endMs` 改成新的时间轴。
